...
def run(session, p_tag_name, p_comment, p_allowed_values):
    ...
    audit_id = session.sql("SELECT HORIZON.AUDIT.SEQ_GOV_AUDIT.NEXTVAL").collect()[0][0]
    fq_tag = "HORIZON.TAGS." + p_tag_name
    ...
    try:
        ...
        session.sql("INSERT INTO HORIZON.TAGS.TAG_REGISTRY (TAG_DATABASE, TAG_SCHEMA, TAG_NAME, TAG_COMMENT, ALLOWED_VALUES, CREATED_BY) VALUES (" + q + HORIZON + q + ", " + q + TAGS + q + ", " + q + p_tag_name + q + ", " + q + p_comment.replace(q,q+q) + q + ", PARSE_JSON(" + q + json.dumps(p_allowed_values if p_allowed_values else []).replace(q,q+q) + q + "), " + q + user + q + ")").collect()
        log(session, audit_id, user, CREATE_TAG, HORIZON, TAGS, p_tag_name, TAG, "Created governance tag", sql, SUCCESS, "")
    except Exception as e:
        result[status] = ERROR
        result[message] = str(e)[:500]
        log(session, audit_id, user, CREATE_TAG, HORIZON, TAGS, p_tag_name, TAG, "Failed to create tag", sql, ERROR, str(e)[:500])
    return result

def log(session, aid, user, etype, db, sch, obj, otype, detail, sql, status, msg):
    q = chr(39)
    try:
        session.sql("INSERT INTO HORIZON.AUDIT.GOV_AUDIT_LOG (AUDIT_ID, INVOKED_BY, EVENT_TYPE, OBJECT_DATABASE, OBJECT_SCHEMA, OBJECT_NAME, OBJECT_TYPE, ACTION_DETAIL, SQL_COMMAND, STATUS, MESSAGE) VALUES (" + str(aid) + ", " + q + user + q + ", " + q + etype + q + ", " + q + db + q + ", " + q + sch + q + ", " + q + obj + q + ", " + q + otype + q + ", " + q + detail.replace(q,q+q) + q + ", " + q + sql.replace(q,q+q)[:2000] + q + ", " + q + status + q + ", " + q + msg.replace(q,q+q)[:500] + q + ")").collect()
    except: pass
';
...
def run(session, p_policy_name, p_data_type, p_template_name, p_authorized_role, p_comment):
    ...
    user = session.sql("SELECT CURRENT_USER()").collect()[0][0]
    audit_id = session.sql("SELECT HORIZON.AUDIT.SEQ_GOV_AUDIT.NEXTVAL").collect()[0][0]
    fq_policy = "HORIZON.POLICIES." + p_policy_name

    tmpl = session.sql("SELECT POLICY_BODY_TEMPLATE FROM HORIZON.POLICIES.MASKING_POLICY_TEMPLATES WHERE TEMPLATE_NAME = " + q + p_template_name + q + " AND DATA_TYPE = " + q + p_data_type + q + " AND IS_ACTIVE = TRUE LIMIT 1").collect()
    if not tmpl:
        result[status] = ERROR
    ...
    try:
        session.sql(sql).collect()
        session.sql("INSERT INTO HORIZON.POLICIES.POLICY_REGISTRY (POLICY_DATABASE, POLICY_SCHEMA, POLICY_NAME, POLICY_TYPE, POLICY_BODY, TARGET_DATA_TYPE, POLICY_COMMENT, CREATED_BY) VALUES (" + q + HORIZON + q + ", " + q + POLICIES + q + ", " + q + p_policy_name + q + ", " + q + MASKING + q + ", " + q + body.replace(q,q+q) + q + ", " + q + p_data_type + q + ", " + q + p_comment.replace(q,q+q) + q + ", " + q + user + q + ")").collect()
        log(session, audit_id, user, CREATE_MASKING_POLICY, HORIZON, POLICIES, p_policy_name, MASKING_POLICY, "Template: " + p_template_name + ", Auth role: " + p_authorized_role, sql, SUCCESS, "")
    except Exception as e:
        result[status] = ERROR
        result[message] = str(e)[:500]
        log(session, audit_id, user, CREATE_MASKING_POLICY, HORIZON, POLICIES, p_policy_name, MASKING_POLICY, Failed, sql, ERROR, str(e)[:500])
    return result
...
GRANT CREATE TAG ON SCHEMA IDENTIFIER($PRISM_HORIZON_DB || '.TAGS') TO ROLE IDENTIFIER($PRISM_GOV_ROLE);
GRANT CREATE MASKING POLICY ON SCHEMA IDENTIFIER($PRISM_HORIZON_DB || '.POLICIES') TO ROLE IDENTIFIER($PRISM_GOV_ROLE);
